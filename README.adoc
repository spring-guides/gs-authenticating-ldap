...
Spring Security. In our example it is optional though, as we are not disabling the
`SecurityAutoConfiguration`, but just override some of it by using the `WebSecurityConfigurerAdapter`.
