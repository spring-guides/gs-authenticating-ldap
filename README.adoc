...
`EmbeddedLdapServerContextSourceFactoryBean` starts the embedded server and creates a
`ContextSource` pointing to it, reading the LDIF file and base DN from
`application.properties`. The `AuthenticationManager` bean uses
`LdapPasswordComparisonAuthenticationManagerFactory` to configure password comparison
against the `userPassword` attribute, with user entries searched by the pattern
`uid={0},ou=people`. The `LdapAuthoritiesPopulator` bean loads group memberships from
`ou=groups`.

== Add Application Properties

Spring LDAP requires that three application properties be set in the `application.properties` file to start the embedded server: the LDIF file to load, the base DN, and the port.
